...
export class QRBuilder{
  private _options : QrSvgOptions | undefined;
  private _config: any | undefined;
  private _type: QRContentType | undefined;
...
public SetQRType(type: QRContentType) : QRBuilder {
  ...
  public SetQRConfig(config:any):QRBuilder{
    this._config = config;
    return this;
  }
  ...
  if(this._type === undefined ||
     ...
     this._config === undefined)
         throw new Error("Not Correct using QRBuilder");

    const formattedContent = formatQRContent(this._type,this._config);
    this.SetDefaultValues(this._options ?? {})
    const qr = qrcodegen.QrCode.encodeText(formattedContent, this._options.ErrorCorrection ?? Ecc.LOW);
    return this.GenerateQRSVG(qr , this._options);
  }

  private SetDefaultValues(options:QrSvgOptions){
